@php
    $saling_price = intval($v->selling_price ?? 0);
    $discount_price = intval($v->discount_price ?? 0);
    $display = $discount_price > 0 ? $discount_price : $saling_price;
@endphp
        ...
        <div class="product-description">
            ...
            <div class="price">
                ...
                @if(theme_setting('products.show_compare_price', true) && $discount_price > 0 && $saling_price > $discount_price)
                    <span class="price-cut">{{ format_amount($saling_price) }}</span>
                @endif
                <span class="new-price">{{ format_amount($display) }}</span>
                @endif
            </div>
        </div>
    </div>
    @if(theme_setting('products.show_add_to_cart', true))
    <div class="product-cart-btn">
        <a href="javascript:void(0)" class="product-btn" onclick="addToCart({{ $v->id }})">Add To Cart</a>
    </div>
    @endif
</div>
