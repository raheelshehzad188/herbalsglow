...
@endif
   ...
   <!-- Icons -->
   <link rel="stylesheet" href="{{ $assets_url }}fonts/font-icons.css">
   <link rel="stylesheet" href="{{ $assets_url }}css/bootstrap.min.css">
   <link rel="stylesheet" href="{{ $assets_url }}css/image-compare-viewer.min.css">
   <link rel="stylesheet" href="{{ $assets_url }}css/swiper-bundle.min.css">
   <link rel="stylesheet" href="{{ $assets_url }}css/animate.css">
   <link rel="stylesheet"type="text/css" href="{{ $assets_url }}css/styles.css"/>
   <style>
    .no-style, 
.no-style * {
    all: unset; /* Sab CSS reset karne ke liye */
}
.no-style b {
...
.no-style em, 
.no-style blockquote {
    all: revert; /* Default browser styles wapas laane ke liye */
}
    ...
    <link rel="shortcut icon" href="{{env('APP_URL')}}{{$Setting->logo1}}">
<link rel="apple-touch-icon-precomposed" href="{{env('APP_URL')}}{{$Setting->logo1}}">
@if(isset($meta_file) && $meta_file)
    @include($meta_file) 
    @else
    @include('meta.default')
...
@endif
    ...
    <?php $Settings = Setting::where(['id'=>'1'])->get(); ?>
    @foreach($Settings as $Setting)
    <!--<link rel="icon" type="image/x-icon" href="{{env('APP_URL')}}/images/favicon.png">-->
    <link rel="shortcut icon" href="{{env('APP_URL')}}{{$Setting->logo1}}" type="image/x-icon"/ >
    @endforeach
...
<body class="preload-wrapper">
    ...
    <div class="tf-toolbar-bottom type-1150">
        ...
        <div class="toolbar-item">
            <a href="{{ url('/my_wishlist') }}">
                <div class="toolbar-icon">
                    ...
                    <div class="toolbar-count">{{ isset($_COOKIE['wishlist'])?count(json_decode($_COOKIE['wishlist'])):0 }}</div>
                </div>
        ...
        <div class="toolbar-item">
            <a href="#shoppingCart" data-bs-toggle="modal">
                <div class="toolbar-icon">
                    <i class="icon-bag"></i>
                    <div class="toolbar-count">{{ Session::has('cart') ? App\Helpers\Cart::qty() : 0 }}</div>
                </div>
        ...
        <div class="mb-canvas-content">
            <div class="mb-body">
                    <ul class="nav-ul-mb" id="wrapper-menu-navigation">
                        ...
                        {{ url('/')}}/{{$v->slug}}
                        @endif
                                    " class="mb-menu-link">{{$v->name}}</a></li>
                        @endforeach                            </ul>
                <div class="mb-other-content">
                    <div class="d-flex group-icon">
                        <a href="{{ url('my_wishlist'); }}" class="site-nav-icon"><i class="icon icon-heart"></i>Wishlist</a>
                    </div>
                    <div class="mb-notice">
                        <?php $setting = DB::table('setting') 
    ->where('id', '=', '1')
    ->first();?>
                    ...
                    <ul class="mb-info">
                        <li>{!!$setting->homepage_footer !!}</li>
                        <li>Email: <b>{{$setting->email}}</b></li>
                        <li>Phone: <b>{{$setting->phone}}</b></li>
                    </ul>
                </div>
            </div>
            <div class="mb-bottom">
            ...
            <header class="tf-search-head">
                ...
                <div class="tf-search-sticky">
                    <form class="tf-mini-search-frm">
                        <fieldset class="text">
                            <input type="text" placeholder="Search" id="srch" class="" name="text" tabindex="0" value="" aria-required="true" required="">
                        </fieldset>
            ...
            <div class="canvas-body p-0">
                <div class="tf-search-content">
                    <div class="tf-cart-hide-has-results">
                        ...
                        <div class="tf-col-content" id="srch_result"></div>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="tf-mini-cart-main">
                            <div class="tf-mini-cart-sroll">
                                <div class="tf-mini-cart-items">
                                    ...
                                    @php
                                    $tot = $tot + $product->discount_price;
                                    @endphp
                                    <div class="tf-mini-cart-item">
                                        ...
                                        <div class="tf-mini-cart-info">
                                            <a class="title link" href="{{url('/')}}/product/{{$product->slug}}">{{$product->product_name}}</a>
                                            <div class="price fw-6">Rs {{$product->discount_price}}</div>
                        ...
                        <div class="tf-mini-cart-bottom">
                            <div class="tf-mini-cart-bottom-wrap">
                                <div class="tf-cart-totals-discounts">
                                    ...
                                    <div class="tf-totals-total-value fw-6">{{ format_amount($tot); }}</div>
                                </div>
    ...
    <script type="text/javascript" src="{{ $assets_url }}js//swiper-bundle.min.js"></script>
    <script type="text/javascript" src="{{ $assets_url }}js//carousel.js"></script>
    <script type="text/javascript" src="{{ $assets_url }}js//bootstrap-select.min.js"></script>
    <script type="text/javascript" src="{{ $assets_url }}js//lazysize.min.js"></script>
    <script type="text/javascript" src="{{ $assets_url }}js//bootstrap-select.min.js"></script>
    <script type="module" src="{{ $assets_url }}js//image-compare-viewer.js"></script>
    <script type="text/javascript" src="{{ $assets_url }}js//wow.min.js"></script>
    <script type="text/javascript" src="{{ $assets_url }}js//multiple-modal.js"></script>
  ...
  $(document).ready(function() {
    ...
    // اگر سرچ انپٹ خالی ہو تو رزلٹ کلئیر کر دیں
    if (query === "") {
        document.getElementById("srch_result").innerHTML = ' ';
    }

    // AJAX request
    document.getElementById("srch_result").innerHTML = '<img src="https://htmlburger.com/blog/wp-content/uploads/2021/07/The-Best-50-Website-Preloaders-Around-the-Web-Example-28.gif" alt="Test Image" style="max-width: 100%; max-height: 100%; object-fit: contain;" />';
    fetch(`{{ url('/search'); }}/${encodeURIComponent(query)}`)
        .then(response => response.text())
        .then(data => {
            document.getElementById("srch_result").innerHTML = data;
        })
      ...
      $(document).ready(function(){ 
          Swal.fire({
  title: "Good job!",
  text: 'Product addd to cart successfully!',
  icon: "success"
});
      });
      @endif
